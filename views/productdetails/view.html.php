    ...
    public $rent_1 = 0;
		function display($tpl = null) {
			...
			if(($fxbot_product !== false) && is_object($fxbot_product) && isset($fxbot_product->id) && ($fxbot_product->id > 0)){
                            $GLOBALS['fxbotmarket_type_of_product'] = $fxbot_product->typeofproduct > 0? $fxbot_product->typeofproduct: 1;
                            //Signal = 1,  Robot (MT4 EA) =2,Robot (MT5 EA) = 3, Indicator = 4,Script = 5,Software = 6,eBook (PDF) = 7, TAP = 1000
                            $fx_product = new stdClass();
                            $fx_product->id_product = $fxbot_product->id;
                            $fx_product->price = $fxbot_product->price;
                            ...
                            $fx_product->trial_period = $fxbot_product->trial_period;
                            $GLOBALS['fxbotmarket_product'] = $fx_product;
                        }else{
                            $GLOBALS['fxbotmarket_type_of_product'] = 1;//Signal by default
                        }
...
//This must be loaded after the customfields are rendered (they may need to overwrite the handlers)
			...
			vmJsApi::jPrice();
			$this->_path['template'][1]=JPATH_ROOT."/plugins/system/vmfxbot/views/productdetails/tmpl/";//bpm
			if(is_object($fxbot_product) && isset($fxbot_product->typeofproduct) && $fxbot_product->typeofproduct == 1){
                            $tpl = 'signal';
                        }elseif(is_object($fxbot_product) && isset($fxbot_product->typeofproduct) && $fxbot_product->typeofproduct == 1001){
                            $tpl = 'blockpad';
                        }
                        parent::display($tpl);
    }
