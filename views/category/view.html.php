...
/*		[metadesc] => menu-meta_description
		...
		if (VmConfig::get ('jdynupdate', TRUE)) {
			vmJsApi::jDynUpdate();
                ...
                $this->monthly_flat_fee = FxbotmarketProduct::getPaypalMontlyFlatFee();  
                $this->fxproducts = $this->getFxbotFilesProducts($ids);
                //$GLOBALS['fxproducts_list'] = $this->fxproducts;
                //var_dump($this->fxproducts);
                $this->_path['template'][1]=JPATH_ROOT."/plugins/system/vmfxbot/views/category/tmpl/";//bpm
		parent::display($tpl);
	}
	...
	public function setCanonicalLink($tpl,$document,$categoryId,$manId){
        ...
        public function getFxbotFilesProducts($vids = array()){
            $db = JFactory::getDbo();
            if(count($vids) > 0){
            $qr = 'SELECT a.id, a.typeofproduct,a.product_id,a.price,a.rent1,'
                    . 'a.rent3,a.rent6,a.rent12,a.trial_period, b.id as demo_id '
                    . ' FROM #__fxbotmarketx_files_products as a LEFT JOIN  '
                    . ' #__fxbotmarketx_demo_files as b ON a.id = b.id_product WHERE '
                    . ' product_id IN ('. implode(',', $vids).')';
            //var_dump($qr);
            $db->setQuery($qr);
            $fxproducts = $db->loadObjectList('product_id');
            //var_dump($fxproducts);
            /*$result = array();
            foreach($fxproducts as $prod){
