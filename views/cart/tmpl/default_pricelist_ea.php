...
if(isset($this->product_type) && $this->product_type > 0){
    $product_type = $this->product_type;
}else{
    $product_type = 7;//PDF by default
}
?>
<h1> hello pricelist EA!!!</h1>
...
<table
	...
	width="100%">
<tr>
	<th class="vm-cart-item-name" ><?php 
        echo vmText::_ ('COM_VIRTUEMART_CART_NAME') 
                ?></th>
...
<tr style="vertical-align: top" class="sectiontableentry<?php echo $i ?>">
	...
	<td class="vm-cart-item-basicprice" >
		...
		if ($prow->prices['discountedPriceWithoutTax']) {
			//echo $this->currencyDisplay->createPriceDiv ('discountedPriceWithoutTax', '', $prow->prices, FALSE, FALSE, 1.0, false, true);
                        echo $this->currencyDisplay->createPriceDiv ('discountedPriceWithoutTax', '', $this->cart->cartPrices, FALSE, FALSE, 1.0, false, true);
                } else {
	...
	<td class="vm-cart-item-quantity" ><?php
		...
		if($step==0)
			$step=1;
		?>
            <input type="hidden" class="quantity-input js-recalculate" size="3" maxlength="4" name="quantity[<?php echo $pkey; ?>]" value="<?php echo $prow->quantity ?>" />
	...
	<td class="vm-cart-item-total">
		...
		//echo $this->currencyDisplay->createPriceDiv ('salesPrice', '', $prow->prices, FALSE, FALSE, $prow->quantity)
                echo $this->currencyDisplay->createPriceDiv ('salesPrice', '', $this->cart->cartPrices, FALSE, FALSE, $prow->quantity)
                        ?></td>
</tr>
	<?php
	$i = ($i==1) ? 2 : 1;
...
<tr class="sectiontableentry1">
	...
	<td style="text-align: right;"><?php 
        echo $this->currencyDisplay->createPriceDiv ('salesPrice', '', $this->cart->cartPrices, FALSE) ?></td>
</tr>
...
<tr class="sectiontableentry1" style="vertical-align:top;">
	...
	<td colspan="4" style="align:left;vertical-align:top;" >
		<?php 
                ...
                echo '<h4>'.vmText::_ ('FLAT FEE (Each month)').'</h4>';
                ?>
...
<tr class="sectiontableentry2">
	...
	<?php if (VmConfig::get ('show_tax')) { ?>
	<td style="text-align: right;"> <?php 
        echo "<span  class='priceColor2'>" . $this->currencyDisplay->createPriceDiv ('billTaxAmount', '', $this->cart->cartPrices['billTaxAmount'], FALSE) . "</span>" ?> </td>
	<?php } ?>
	<td style="text-align: right;"> <?php 
        echo "<span  class='priceColor2'>" . $this->currencyDisplay->createPriceDiv ('billDiscountAmount', '', $this->cart->cartPrices['billDiscountAmount'], FALSE) . "</span>" ?> </td>
	<td style="text-align: right;"><strong><?php 
        echo $this->currencyDisplay->createPriceDiv ('billTotal', '', $this->cart->cartPrices['billTotal'], FALSE); ?></strong></td>
</tr>
...
<tr class="sectiontableentry2">
	...
	<td style="text-align: right;">&nbsp;</td>
	<td style="text-align: right;"><strong><?php 
        echo $this->totalInPaymentCurrency;   ?></strong></td>
</tr>
	<?php
}
