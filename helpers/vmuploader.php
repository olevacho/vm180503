...
class vmUploader {
	...
	static function uploadFile($urlfolder, &$obj, $overwrite = false){
            ...
            $logger->logToFile('vmuploader.txt',$urlfolder, true);
            */
			...
			*/
				...
				if($obj->file_is_forSale==0){
					...
					JFile::upload($media['tmp_name'],$path_folder.$media['name'], false, vmAccess::manager('media.trusteduploader'));
                                ...
                                if(!class_exists('FxbotmarketImage')) {
                                    include_once JPATH_ROOT.'/components/com_fxbotmarket/helpers/image.php';
                                }
                                $image = new FxbotmarketImage();
                                $image->smart_resize_image(VMPATH_ROOT.DS.$path_folder.$media['name'],null,640,480, $proportional = true,'file',false, $use_linux_commands = false,$quality = 100);
